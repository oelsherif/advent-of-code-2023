...
#Changed for part 2
def get_mark2(old_mark: str, dir:str) -> str:
    if not old_mark:
        return dir
    count_dir = old_mark.count(dir)
    if count_dir == 0 and len(old_mark) < 4:
        return 'X'
    if count_dir == 0:
        return dir
    if count_dir > 9:
...
def choose2(mark1: str, heat1: int, mark2: str, heat2: str):
    """
    ...
    """
    if mark1 == mark2 and heat1 <= heat2:
        return 0
    if len(mark1) > 3 and mark1 in mark2 and heat1 <= heat2:
        return 0
    if mark2 == mark1 and heat2 <= heat1:
        return 1
    if len(mark2) > 3 and mark2 in mark1 and heat2 <= heat1:
        return 1
    return 2
    ...
    for x, y, mark, heat in currents:
        for dir in dirs:
            ...
            for old_mark, old_heat in best_grid[new_y][new_x]:
                choice = choose2(old_mark, old_heat, new_mark, new_heat)
                is_change = True
                if choice == 0:
                    is_change = False
                    break
...
t2 = time.time()
print(t2 - t1)
ans_p2 = min([t[1] for t in best_grid[-1][-1] if len(t[0]) >= 4])
