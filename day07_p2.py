...
def hand_type(hand):
    '''Takes hand and return its type, with 5 of a kind getting rank 6, and high card getting rank 0, includes Joker rule'''
    card_counts = Counter(hand)
    J_count = 0
    if 'J' in hand and card_counts['J'] < 5:
        J_count = card_counts['J']
        card_counts['J'] = 0
    freq = sorted(card_counts.values(), reverse = True)
    freq[0] += J_count
    if freq[0] == 5:
        return 6
    elif freq[0] == 4:
...
max_hand_value = 13 ** 5
card_value = {card: i for i, card in enumerate("J23456789TQKA")}
